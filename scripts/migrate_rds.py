...
def wait_for(describe_fn, check_fn, label, interval=30, timeout=3600):
    elapsed = 0
...
def migrate_rds(cfg, db_id, target_instance_class, target_subnet_group, dry_run=False):
    ...
    wait_for(
        lambda: src_rds.describe_db_snapshots(DBSnapshotIdentifier=snap_id),
        lambda r: r["DBSnapshots"][0]["Status"] == "available",
        f"Snapshot {snap_id} available",
    )
    snap_arn = src_rds.describe_db_snapshots(
        DBSnapshotIdentifier=snap_id
    )["DBSnapshots"][0]["DBSnapshotArn"]
    print(f"  ✅ Snapshot: {snap_arn}")

    # Step 2: Share snapshot with target account
    print(f"\n[2/5] Sharing snapshot with account {tgt['account_id']}...")
    src_rds.modify_db_snapshot_attribute(
        DBSnapshotIdentifier=snap_id,
    ...
    wait_for(
        lambda: tgt_rds.describe_db_snapshots(DBSnapshotIdentifier=target_snap_id),
        lambda r: r["DBSnapshots"][0]["Status"] == "available",
        f"Target snapshot {target_snap_id} available",
        interval=60,
    )
    ...
    restore_params = {
        ...
        "DBSnapshotIdentifier": target_snap_id,
    ...
    wait_for(
        lambda: tgt_rds.describe_db_instances(DBInstanceIdentifier=target_db_id),
        lambda r: r["DBInstances"][0]["DBInstanceStatus"] == "available",
        f"DB {target_db_id} available",
        interval=60,
    )
    endpoint = tgt_rds.describe_db_instances(
        DBInstanceIdentifier=target_db_id
    )["DBInstances"][0]["Endpoint"]
    print(f"  ✅ Restored: {target_db_id}")

    # Step 5: Summary
    print(f"\n[5/5] ✅ Migration complete!")
    print(f"  Endpoint: {endpoint['Address']}:{endpoint['Port']}")
    print(f"  Region:   {tgt['region']}")
    print(f"  ⚠️  Update security groups, parameter groups, and app connection strings.")
...
def main():
    ...
    parser.add_argument("-d", "--db-instance-id", help="Override: single DB instance ID")
    parser.add_argument("--instance-class", default="db.r6g.large", help="Target instance class")
    parser.add_argument("--subnet-group", help="Target DB subnet group name")
    ...
    if args.db_instance_id:
        instances = [{"db_instance_id": args.db_instance_id,
                      "target_instance_class": args.instance_class,
    ...
    for db in instances:
        try:
            migrate_rds(
                cfg, db["db_instance_id"],
                db.get("target_instance_class", args.instance_class),
                db.get("target_subnet_group", args.subnet_group),
